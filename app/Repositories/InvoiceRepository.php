...
class InvoiceRepository extends BaseRepository
{
    ...
    public function subcription(array $input, $data)
    {

        $user_count = Session::get('user_count');


        $package = Package::where('id', $input['package_id'])->get();
        $product_id = '';
        $currency_id = '';
        $price = '';
        $plan_name = '';
        ...
        foreach ($package as $pack) {
            $product_id = $pack->id;
            $currency_id = $pack->currency_id;
            $price = $pack->price;
            $plan_name = $pack->plan_name;
            $recurring_cycle = $pack->package_duration;
        }
        ...
        $product_id;
        $status = 1;
        $recurring_status = $data['recurringArr']['1'];
        ...
        $date = Carbon::createFromFormat('Y-m-d H:i:s', $invoice_date)->format('Y-m-d');
        $due_date = Carbon::createFromFormat('Y-m-d H:i:s', $due_date)->format('Y-m-d');
    ...
    public function saveSubcription(array $input)
    {
        $client_id = auth()->user()->client->id;
        $subcription = subcription::where('client_id', $client_id)->get();

        if ($subcription->isEmpty()) {
            try {

                DB::beginTransaction();
                $user = subcription::create($input);
                DB::commit();
                return true;
            } catch (\Exception $e) {
                DB::rollBack();
    ...
    public function saveInvoice(array $input)
    {
        ...
        try {
            ...
            $input['final_amount'] = $input['amount']*$input['quantity'];
            ...
            if ($invoiceExist) {
                $inoviceStatus = Invoice::where('invoice_id', $client_id)->where('status', 1)->get()->toArray();
                $inoviceStatusPending = Invoice::where('invoice_id', $client_id)->where('status', 7)->get()->toArray();
                $subcription = subcription::where('client_id', $client_id)->where('package_id', $input['product_id'])->get()->toArray();
                if ($subcription != null) {

                    throw new UnprocessableEntityHttpException('Invoice id already exist');

                } elseif ($inoviceStatus != null) {

                    throw new UnprocessableEntityHttpException('Active Your package for swtiching id already exist ');

                } elseif ($inoviceStatusPending != null) {

                    throw new UnprocessableEntityHttpException('You have already pending inovice');

                } else {
                    $a1 = $input;
                    $a2 = array("status" => "7");
                    $input = array_replace($a1, $a2);
                }

            }


            if($input['recurring_cycle']==14){
            ...
            foreach ($invoiceItemInput as $key => $data) {
                ...
                if (isset($data['product_name']) != null) {
                ...
                $invoiceItemTaxIds = ($input['tax_id'] != 0) ? $input['tax_id'] : $input['tax_id'] = [0 => 0];
                // $invoiceItemTaxIds = ($input['tax_id'][$key] != 0) ? $input['tax_id'][$key] : $input['tax_id'][$key] = [0 => 0];
                $invoiceItemTaxes = ($input['tax'] != 0) ? $input['tax'] : $input['tax'] = [0 => null];
    ...
    public function prepareInputForInvoiceItem(array $input): array
    {
        ...
        if (is_string($input['price'])) {
            ...
            foreach ($input as $key => $data) {
                ...
                if (!(isset($items[$key]['price']) && $key == 'price')) {
                    continue;
                }
                $items[$key]['price'] = removeCommaFromNumbers($items[$key]['price']);
            }
